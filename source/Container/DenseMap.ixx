...
export module DeerContainer:DenseMap;
import :CompressedPair;
import :Pair;
import :Vector;
import DeerAllocator;
...
struct SDenseMapNode final
{
    ...
    SDenseMapNode(const std::size_t pos, Args&&... args) : next{pos}, element{std::forward<Args>(args)...}
    {
    }

    template <typename Allocator, typename... Args>
    SDenseMapNode(std::allocator_arg_t, const Allocator& allocator, const std::size_t pos, Args&&... args) :
        next{pos}, element{DeerAllocator::MakeObjUsingAllocator<value_type>(allocator, std::forward<Args>(args)...)}
    {
    }

    template <typename Allocator>
    SDenseMapNode(std::allocator_arg_t, const Allocator& allocator, const SDenseMapNode& other) :
        next{other.next}, element{DeerAllocator::MakeObjUsingAllocator<value_type>(allocator, other.element)}
    {
    }

    template <typename Allocator>
    SDenseMapNode(std::allocator_arg_t, const Allocator& allocator, SDenseMapNode&& other) :
        next{other.next}, element{DeerAllocator::MakeObjUsingAllocator<value_type>(allocator, std::move(other.element))}
    {
    }
...
class CDenseMapIterator final
{
  ...
  public:
    ...
    constexpr CDenseMapIterator() noexcept : m_itr{} {}

    explicit constexpr CDenseMapIterator(const Itr iter) noexcept : m_itr{iter} {}

    template <typename Other>
        requires(!std::is_same_v<Itr, Other>) && std::is_constructible_v<Itr, Other>
    constexpr explicit CDenseMapIterator(const CDenseMapIterator<Other>& other) noexcept : m_itr{other.iter}
    {
    }
...
class CDenseMapLocalIterator final
{
  ...
  public:
    ...
    constexpr CDenseMapLocalIterator() noexcept : m_itr{}, m_offset{} {}

    constexpr CDenseMapLocalIterator(Itr iter, const std::size_t pos) noexcept : m_itr{iter}, m_offset{pos} {}
 ...
 * @since 0.1.0
 *
 * @note This class relies on `SPair`, `SCompressedPair`, and `CVector` for internal storage and handling,
 * offering a more customized approach than standard containers.
 * @warning CDenseMap has not been tested on thread safety, use with causion.
 *
 * @see [std::map](https://en.cppreference.com/w/cpp/container/map), [skypjack/entt](https://github.com/skypjack/entt), SPair,
 * SCompressedPair, CVector
 */
export template <typename Key,
                 typename Type,
                 typename Hash      = std::identity,
                 typename Equals    = std::equal_to<>,
                 typename Allocator = std::allocator<SPair<const Key, Type>>>
class CDenseMap final
{
  ...
  public:
    ...
    ///@{
    /**
     * @brief Default member-functions. Initializes the map with default settings.
     */
    constexpr CDenseMap() : CDenseMap{key_compare{}} {}

    /**
     * @brief Constructs the map with a specified allocator.
     * @param alloc Allocator used for memory allocation within the map.
     */
    constexpr explicit CDenseMap(const allocator_type& alloc) : CDenseMap{key_compare{}, alloc} {}
     ...
     */
    constexpr CDenseMap(size_type cnt, const allocator_type& allocator) : CDenseMap{cnt, hasher_type{}, allocator} {}
    ...
    constexpr explicit CDenseMap(const size_type cnt, const hasher_type& hash, const key_compare& compare, const allocator_type& allocator) :
        m_sparse{allocator, hash}, m_packed{allocator, compare}
    {
        Rehash(cnt);
    }
    ...
    constexpr CDenseMap(InputIt               first,
                        InputIt               last,
    ...
    /**
     * @brief Copy member-functions with allocator. Creates a copy of another map, using a specific allocator.
     * @param other The map to copy from.
     * @param allocator Allocator used for memory allocation within the map.
     */
    constexpr CDenseMap(const CDenseMap& other, const allocator_type& allocator) :
        m_sparse{std::piecewise_construct,
                 std::forward_as_tuple(other.m_sparse.first, allocator),
                 std::forward_as_tuple(other.m_sparse.second)},
        m_packed{std::piecewise_construct,
                 std::forward_as_tuple(other.m_packed.first, allocator),
    ...
    /**
     * @brief Move member-functions with allocator. Transfers resources from another map, using a specific allocator.
     * @param other The map to move from.
     * @param allocator Allocator used for memory allocation within the map.
     */
    constexpr CDenseMap(CDenseMap&& other, const allocator_type& allocator) :
        m_sparse{std::piecewise_construct,
                 std::forward_as_tuple(std::move(other.m_sparse.first)),
                 allocator,
                 std::forward_as_tuple(std::move(other.m_sparse.second))},
        m_packed{std::piecewise_construct,
                 std::forward_as_tuple(std::move(other.m_packed.first)),
                 allocator,
     ...
     * @param alloc Allocator used for memory allocation within the map.
     */
    constexpr CDenseMap(std::initializer_list<value_type> init,
     ...
     *
     * This member-functions initializes the dense map by taking a range defined by two iterators.
     *
     ...
     */
    template <typename InputIt>
    constexpr CDenseMap(InputIt first, InputIt last, const allocator_type& alloc) : CDenseMap(first, last, key_compare{}, alloc)
    {
    }
     ...
     *
     * This member-functions initializes the dense map from a range compatible with the specified value type.
     *
     ...
     */
    template <ContainerCompatibleRange<value_type> R>
    constexpr CDenseMap(std::from_range_t /*from_range*/,
                        R&&                   range,
                        const key_compare&    comp  = key_compare{},
                        const allocator_type& alloc = allocator_type{}) :
        m_sparse{alloc, hasher_type{}}, m_packed{alloc, comp}
    {
     ...
     * @brief Accesses the element with the specified key.
     *
     * @param key The key of the element to find. The element gets created if it does not exists.
     * @return A const reference to the mapped value of the specified key.
     */
     ...
     * @param key The key of the element to find. The element gets created if it does not exists.
     * @return A const reference to the mapped value of the specified key.
     * @overload auto operator[]( const key_type& key )
     */
    ...
    /**
     * @brief returns an iterator to the ending.
     */
    ...
    /**
     * @brief returns a reverse iterator to the ending.
     */
    ...
    {
        for (size_type* pCurr{&m_sparse.first[KeyToBucket(key)]}; *pCurr != (std::numeric_limits<size_type>::max)();
    ...
    {
        for (size_type* pCurr{&m_sparse.first[KeyToBucket(std::forward<K>(key))]}; *pCurr != (std::numeric_limits<size_type>::max)();
             pCurr = &m_packed.first[*pCurr].next)
        {
    ...
    void swap(CDenseMap& other) noexcept(std::allocator_traits<Allocator>::is_always_equal::value
                                         && std::is_nothrow_swappable<Equals>::value)
    {
        using std::swap;
  ...
  private:
    ...
    constexpr auto InsertOrDoNothing(Key2&& key, Args&&... args) -> SPair<iterator, bool>
    {
        ...
        m_packed.first.emplace_back(m_sparse.first[index],
...
namespace std
{
export template <typename Key, typename Type, typename Hash, typename Equals, typename Allocator>
void swap(DeerContainer::CDenseMap<Key, Type, Hash, Equals, Allocator>& lhs,
          DeerContainer::CDenseMap<Key, Type, Hash, Equals, Allocator>& rhs) noexcept(noexcept(lhs.swap(rhs)))
{
    lhs.swap(rhs);
}

export template <typename Key, typename Value, typename Allocator>
struct uses_allocator<DeerContainer::CDenseMap<Key, Value>, Allocator> : std::true_type
{
