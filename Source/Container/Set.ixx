...
export module DeerContainer.Set;
import DeerContainer.Pair;
import DeerContainer.ValueTraits;
...
public:
    ...
    constexpr CDenseSetIterator() noexcept
        : m_itr{}
    {
    }

    explicit constexpr CDenseSetIterator( const Itr iter ) noexcept
        : m_itr{iter}
    {
    }
    ...
    constexpr explicit CDenseSetIterator( const CDenseSetIterator<Other>& other ) noexcept
        : m_itr{other.iter}
    {
    }
...
public:
    ...
    constexpr CDenseSetLocalIterator() noexcept
        : m_itr{}
        , m_offset{}
    {
    }

    constexpr CDenseSetLocalIterator( Itr iter, const std::size_t pos ) noexcept
        : m_itr{iter}
        , m_offset{pos}
    {
    }
...
public:
    ...
    constexpr CSparseSetIterator() noexcept
        : m_packed{}
        , m_offset{}
    {
    }

    constexpr CSparseSetIterator( const Container& ref, const difference_type idx ) noexcept
        : m_packed{&ref}
        , m_offset{idx}
    {
    }

    constexpr auto operator++() noexcept -> CSparseSetIterator&
    {
        ++m_offset;
        return *this;
    }

    constexpr auto operator++( int ) const noexcept -> CSparseSetIterator
    {
        CSparseSetIterator orig{*this};
        ++*this;
        return orig;
    }
    ...
    constexpr auto operator+( difference_type value ) const noexcept -> CSparseSetIterator
    {
        CSparseSetIterator temp{*this};
        temp += value;
        return temp;
    }

    constexpr auto operator+=( difference_type value ) noexcept -> CSparseSetIterator&
    {
        m_offset += value;
        return *this;
    }

    constexpr auto operator-( const difference_type value ) const noexcept -> CSparseSetIterator
    {
        return *this + ( -value );
    }

    constexpr auto operator-=( const difference_type value ) noexcept -> CSparseSetIterator&
    {
        *this += ( -value );
        return *this;
    }

    friend constexpr auto operator-( const CSparseSetIterator& lhs, const CSparseSetIterator& rhs ) noexcept -> difference_type
    {
        return lhs.m_offset - rhs.m_offset;
    }

    friend constexpr auto operator+( difference_type lhs, CSparseSetIterator rhs ) noexcept -> CSparseSetIterator
    {
        rhs += lhs;
        return rhs;
    }
    ...
    {
        return m_packed ? std::data( m_packed ) : nullptr;
    }
...
public:
    ...
    constexpr CDenseSet()
        : CDenseSet{key_compare{}}
    {
    }

    constexpr explicit CDenseSet( const allocator_type& alloc )
        : CDenseSet{key_compare{}, alloc}
    {
    }
    ...
    constexpr CDenseSet( const size_type cnt, const allocator_type& alloc )
        : CDenseSet{cnt, hash_type{}, alloc}
    {
    }
    ...
    constexpr CDenseSet( InputIt first, InputIt last, const allocator_type& alloc )
        : CDenseSet{first, last, key_compare{}, alloc}
    {
    }

    template<class InputIt>
    constexpr CDenseSet( InputIt first, InputIt last, const hash_type& hash = hash_type{}, const key_compare& comp = key_compare{}, const allocator_type& alloc = allocator_type{} )
        : m_sparse{alloc, hash}
        , m_packed{alloc, comp}
    {
        insert( first, last );
    ...
    constexpr CDenseSet( const CDenseSet& other, const allocator_type& alloc )
        : m_sparse{std::piecewise_construct, std::forward_as_tuple( other.m_sparse.first(), alloc ), std::forward_as_tuple( other.m_sparse.second() )}
        , m_packed{std::piecewise_construct, std::forward_as_tuple( other.m_packed.first(), alloc ), std::forward_as_tuple( other.m_packed.second() )}
        , m_threshold{other.m_threshold}
    {
    }

    constexpr CDenseSet( CDenseSet&& other, const allocator_type& alloc )
        : m_sparse{std::piecewise_construct, std::forward_as_tuple( std::move( other.m_sparse.first() ), alloc ), std::forward_as_tuple( std::move( other.m_sparse.second() ) )}
        , m_packed{std::piecewise_construct, std::forward_as_tuple( std::move( other.m_packed.first() ), alloc ), std::forward_as_tuple( std::move( other.m_packed.second() ) )}
        , m_threshold{other.m_threshold}
    {
    }
    ...
    constexpr CDenseSet( std::initializer_list<value_type> init, const hash_type& hasher, const key_compare& comp, const allocator_type& alloc )
    ...
    template<ContainerCompatibleRange<value_type> R>
    constexpr CDenseSet( std::from_range_t /*from_range*/, R&& range, const key_compare& comp = key_compare{}, const allocator_type& alloc = allocator_type{} )
        : m_sparse{alloc, hash_type{}}
        , m_packed{alloc, comp}
    {
    ...
    {
        for ( size_type* pCurr{&m_sparse.first()[ ValueToBucket( key ) ]}; *pCurr != ( std::numeric_limits<size_type>::max )(); pCurr = &m_packed.first()[ *pCurr ].first )
        {
    ...
    {
        for ( size_type* pCurr{&m_sparse.first()[ ValueToBucket( std::forward<V>( value ) ) ]}; *pCurr != ( std::numeric_limits<size_type>::max )(); pCurr = &m_packed.first()[ *pCurr ].first )
        {
...
private:
    using sparse_container_type = CVector<typename alloc_traits::pointer, typename alloc_traits::template rebind_alloc<typename alloc_traits::pointer>>;
    using packed_container_type = CVector<Type, Allocator>;
...
private:
    ...
    const type_info* m_info{nullptr};
    size_type m_head{};
    deletion_policy m_mode {deletion_policy::swap_and_pop} ;
};
