...
public:
    ...
    constexpr CVectorIterator() noexcept
        : m_itr{}
    {
    }

    constexpr explicit CVectorIterator( Itr value ) noexcept
        : m_itr{value}
    {
    }
    ...
    CVectorIterator( const CVectorIterator<Other>& other ) noexcept
        : m_itr{other.base()}
    {
    }
...
public:
    ...
    constexpr explicit CVector( const Allocator& alloc ) noexcept
        : m_max{nullptr, alloc}
    {
    }

    constexpr CVector( size_type count, const value_type& value, const Allocator& alloc = Allocator() )
        : m_max{nullptr, alloc}
    {
        if ( count < 0 )
        {
    ...
    constexpr explicit CVector( size_type count, const Allocator& alloc = Allocator() )
        : m_max{nullptr, alloc}
    {
        Allocate( count );
    }
    ...
    constexpr CVector( InputIt first, InputIt last, const Allocator& alloc = Allocator() )
        : m_max{nullptr, alloc}
    {
    ...
    {
        return std::min<size_type>( std::allocator_traits<allocator_type>::max_size( m_max.second() ), std::numeric_limits<difference_type>::max() );
    }
    ...
    constexpr auto insert( const_iterator pos, const_reference value ) -> iterator
    {
        ...
        if ( m_end < m_max.first() )
        {
            ...
            else
            {
                ...
                if ( std::less()( std::to_address( pntr ), std::addressof( value ) ) && !std::less()( std::to_address( m_end ), std::addressof( value ) ) )
                {
                    ++pntrTo;
                }
                *pntr = *pntrTo;
